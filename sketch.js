...
//Condiciones para estado de juego PLAY
  if (gameState === PLAY){
      ...
      spawnClouds();
        ...
        //mover el suelo
      ground.velocityX = -(6 + 1.5*score/200); 
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.scale = 0.6;
    cloud.velocityX = -(6 + 1.5*score/200);
    cloud.lifetime = (220 - 1.5*score/200);
...
function spawnObstacles(){
  if (frameCount % 60 === 0){
    ...
    obstacle.velocityX = -(6 + 1.5*score/200); 
    obstacle.scale = 0.5;

    
    //dar tiempo de vida a los obstaculos
    obstacle.lifetime = (220 - 1.5*score/200);
